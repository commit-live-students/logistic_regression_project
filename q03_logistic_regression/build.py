...
def logistic_regression(X_train, X_test, y_train, y_test):
    SCL=['ApplicantIncome', 'CoapplicantIncome', 'LoanAmount']
    stand_scale= StandardScaler()
    X_train.loc[:, SCL] = stand_scale.fit_transform(X_train.loc[:, SCL])
    X_test.loc[:, SCL] = stand_scale.fit_transform(X_test.loc[:, SCL])
